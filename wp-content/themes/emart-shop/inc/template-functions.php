 ...
 * @return array
 */
function emart_shop_body_classes( $classes ) {
...
add_filter( 'body_class', 'emart_shop_body_classes' );

/**
 * Add a pingback url auto-discovery header for single posts, pages, or attachments.
 */
function emart_shop_pingback_header() {
	if ( is_singular() && pings_open() ) {
		printf( '<link rel="pingback" href="%s">', esc_url( get_bloginfo( 'pingback_url' ) ) );
	}
}
add_action( 'wp_head', 'emart_shop_pingback_header' );

if ( ! function_exists( 'emart_shop_alowed_tags' ) ) :
	/**
	 * @see diet_shop_alowed_tags().
	 */
function emart_shop_alowed_tags() {
	...
	return apply_filters( 'emart_shop_alowed_tags', $tags );
...
if ( ! function_exists( 'emart_shop_walker_comment' ) ) : 
	/**
	...
	function emart_shop_walker_comment($comment, $args, $depth) {
		if ( 'div' === $args['style'] ) {
			$tag       = 'div';
			$add_below = 'comment';
		} else {
...
class emart_shop_navwalker extends Walker_Nav_Menu {
		/**
		 * Menu Fallback
		 * =============
...
if( !function_exists('emart_shop_elementor_editor_simplify') ){
	
	function emart_shop_elementor_editor_simplify(){
	...
	add_action( 'elementor/editor/init', 'emart_shop_elementor_editor_simplify');
